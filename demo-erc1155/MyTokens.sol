...
contract MyToken is ERC1155, ERC165, ERC1155Metadata_URI {
    ...
    address payable private _ownerCtt;

    mapping(uint256 => mapping(address => uint256)) private _balances; //tokenId (owner => balance)
    mapping(address => mapping(address => bool)) private _approvals; //owner (operator => isApproved)

    constructor() {
        _ownerCtt = payable(msg.sender);
    }

    function withdraw() external {
        require(_ownerCtt == msg.sender, "You do not have permission");
        uint256 amount = address(this).balance;
        (bool success, ) = _ownerCtt.call{value: amount}("");
    ...
    function balanceOfBatch(
        ...
        require(
            ...
            "_owners x _ids length mismatch"
        );
    ...
    function _addessesVerifications(
        address _from,
        address _to
    ) private view returns (bool) {
        require(_from != address(0) && _from != _to, "Invalid _from");
    ...
    function safeTransferFrom(
        ...
        _addessesVerifications(_from, _to);
    ...
    function safeBatchTransferFrom(
        ...
        require(_ids.length == _values.length, "_ids x_values length mismatch");
        _addessesVerifications(_from, _to);
