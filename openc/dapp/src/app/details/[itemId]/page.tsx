...
import { loadDetails, NFT, buyNFT } from "@/app/services/Web3Services";
...
function Details() {
  ...
  useEffect(() => {
    ...
    loadDetails(Number(itemId))
      .then(nft => setNft(nft))
      .catch(err => setMessage(`Error loading details: ${err.message}`));
  }, [params.itemId]);

  function btnBuyClick() {
    if (!nft) return;

    setMessage("Processing your purchase... Please wait.");
    buyNFT(nft)
      .then(tx => {
        setMessage("Transaction successful! Redirecting...");
